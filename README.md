<h1>Учебный проект для Маскимастер</h1>
<ul>
<li>Верстка макета <a href="https://www.figma.com/design/2dEZIWvZyomsUEFaPYo7zJ/design-maroon?node-id=77-20&t=6JvcSrdP0iJbAnKs-0"> по шаблону с Figma</a> </li>
<li>Использованы моковые данные для наполнения карточек товаров</li>
<li>Реализация роутинга, пагинации, свайпера</li>
<li>Реализация фильтра по различным категориям товаров</li>
<li>Реализация бургер-меню и аккордеона</li>
<li>Внедрение интерактивной карты через iframe</li>
<li>Адаптив для корректной работы на планшете и мобильных устройствах</li>
</ul>

[https://agnezdei.github.io/maroon-new/](https://agnezdei.github.io/maroon-new/)
